...
Вычищенную до нуля область (например, `chat.d.ts`) можно перевести из `warn` в `error`
отдельным блоком в `eslint.config.mjs` (`files: [...]`): новое нарушение там станет ошибкой
сборки. Так чистые области фиксируются по мере миграции.

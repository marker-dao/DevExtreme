...
### Как определить хранимый дефолт

`@default` описывает **хранимый дефолт — то, что вернёт `.option(path)` на свежем инстансе.** Часто это запись в `_getDefaultOptions()`, но во многих компонентах дефолт лежит НЕ там, и греп одного `_getDefaultOptions` даёт ложное «undefined». Проверяй по факту:

- **Коллбэки через регистрацию.** Гриды (`this.createAction('onX')`), Scheduler, FileManager, viz (`_eventsMap`): регистрация действия НЕ задаёт дефолт опции — обёртка кладётся во внутреннее поле, а `.option('onX')` остаётся `undefined`, пока значение явно не записано в дефолты. Такой коллбэк хранит `undefined` (не `null`), даже если в `.d.ts` стоит `@default null`. `null` он хранит только если в каком-то `_getDefaultOptions`/дефолтах модуля есть буквально `onX: null`.
- **Visualization (`js/viz/**`).** Дефолты — в темах (`viz/core/themes/generic/light/*`), не в `_getDefaultOptions`. Тематическое значение round-trip-ится через `.option()` (это хранимый дефолт); значение, применяемое только при рендере (`x ?? 'solid'`), НЕ хранится → `undefined`.
- **Модульные дефолты (гриды).** `_getDefaultOptions` собран из многих модулей (`grid_core/*`, `data_grid/*`, `tree_list/*`) — искать запись нужно по всему дереву виджета.
- **Каст `null as undefined`** на присваивании — хранится всё равно `null`.
- **Точечная функция** вместо `null`/`undefined` (`noop`, `defaultScreenFactorFunc`, `defaultOnIncidentOccurred`) — тип уже корректен (функция), а `@default null` неточен → решает разработчик.

Каждый кейс проверяется атомарно по рантайму; «как у соседнего поля» — не аргумент.
...
**`any`-поле.** `any` НЕ nullable — он *отключает* проверку типа (`any ≠ unknown`), а не «содержит `null`». При `@default null` на `any`-поле **добавляем `| null`**: это документирует реально хранимое значение, а `any | null` для компилятора схлопывается в `any` (правило `no-redundant-type-constituents` в линте не включено — нового замечания не будет). Не путать с алиасом, который уже включает `null` (`DateLike = Date | number | string | null`) — там тип уже корректен, не трогаем (см. «Слепые зоны линтера»).
...
без `@default`. Примеры таких формулировок:
...
> отклоняется от правила для нового кода. **В новом коде** должен быть `undefined`.
...
| `T \| null` | `null` |
| `any` (опция, дефолт `null`) | `null` — добавить `\| null` (для компилятора = `any`) |
...
(для `js/**/*.d.ts`, статус `warn` — видно в редакторе и в `lint-dts`). Держать в голове не нужно.

Реализация — `packages/devextreme/eslint_plugins/`.

**Слепые зоны линтера** (предсказуемые ложные срабатывания — не «чинить» вслепую):
- **не разворачивает алиасы**: `DateLike` (= `Date | number | string | null`) флагается как «нет `null`», хотя `null` уже в типе → правка не нужна (фикс — раскрытие алиасов в самом правиле);
- **не различает `any`**: флагает `any + @default null` — это верно (см. правило `any`), но `any` линтер по сути «не видит».
